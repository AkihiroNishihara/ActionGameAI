...
class GameStage:
    def __init__(self, _screen_class_arg):
        ...
        self.rect_player.update_pos_rect(h.SIZE_BLOCK * _list_pos_player_ini[1], h.SIZE_BLOCK * _list_pos_player_ini[0])
        self.rect_goal.update_pos_rect(h.SIZE_BLOCK * _list_pos_goal_ini[1], h.SIZE_BLOCK * _list_pos_goal_ini[0])
        ...
        while self.is_loop:
            ...
            pygame.draw.rect(self.screen, (200, 200, 200), Rect(0, 0, h.SCREEN_WIDTH, h.SIZE_BLOCK))  # 画面上部の四角形の描写
            ...
            for i, j in itertools.product(range(1, int(h.SCREEN_HEIGHT / h.SIZE_BLOCK)),
                ...
                if _list_info_stage[i][j] == 1:
                    self.screen.blit(self.rect_brick.get_obj(), [h.SIZE_BLOCK * j, h.SIZE_BLOCK * i])
                elif _list_info_stage[i][j] == 2:
                    self.screen.blit(self.rect_goal.get_obj(), [h.SIZE_BLOCK * j, h.SIZE_BLOCK * i])
            # プレイヤー描写
